...
/// `SilentFileStore` so that `CollectionService::delete_book` can complete
/// without panicking on the no-op store.
fn collection_services(ctx: &crate::context::TestContext) -> std::sync::Arc<bb_core::CoreServices> {
...
async fn delete_book_removes_book_record() {
    let ctx = setup().await;
    let svc = collection_services(&ctx);
    let book = fixtures::insert_book(&ctx.repos, "To Delete", BookStatus::Available).await;

    svc.collection_service.delete_book(book.token).await.unwrap();
...
async fn delete_book_preserves_author_with_other_books() {
    ...
    svc.collection_service.delete_book(book1.token).await.unwrap();
...
async fn delete_book_returns_not_found_for_missing_token() {
    ...
    let result = svc.collection_service.delete_book(ghost_token).await;

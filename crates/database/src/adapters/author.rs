...
impl AuthorRepository for AuthorRepositoryAdapter {
    ...
    async fn find_by_name(&self, transaction: &dyn Transaction, name: &str) -> Result<Option<Author>, Error> {
        ...
        Ok(prelude::Authors::find()
            .filter(Expr::expr(Func::lower(Expr::col(authors::Column::Name))).binary(BinOper::Equal, Expr::value(name.to_lowercase())))
            .one(transaction)
            .await
            .map_err(handle_dberr)?
            .map(Into::into))
    ...
    async fn delete_author(&self, transaction: &dyn Transaction, author_id: AuthorId) -> Result<(), Error> {
        ...
        if let Some(existing) = prelude::Authors::find_by_id(author_id as i64).one(transaction).await.map_err(handle_dberr)? {
